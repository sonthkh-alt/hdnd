...
export function EmployeeManager() {
  ...
  const handleAdd = (e: React.FormEvent) => {
    ...
    const newEmp: Employee = {
      ...
      education: formData.education || 'Đại học',
              ...
              {filteredEmployees.map(emp => {
                ...
                return (
                  <tr key={emp.id} className="hover:bg-slate-50 transition-colors">
                    ...
                    <td className="p-4">
                      <button 
                        onClick={() => alert(`Đang phát triển tính năng xem chi tiết cho đồng chí ${emp.name}`)}
      ...
      <Modal isOpen={isAdding} onClose={() => setIsAdding(false)} title="Thêm hồ sơ Cán bộ mới">
        <form onSubmit={handleAdd} className="space-y-4">
          ...
          <div className="flex items-center gap-2 mt-2">
            ...
            />
            <label htmlFor="partyMember" className="text-sm font-medium text-slate-700">Đảng viên Đảng Cộng sản Việt Nam</label>
          </div>
